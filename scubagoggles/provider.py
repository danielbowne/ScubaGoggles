...
from scubagoggles.utils import (
    create_subset_inverted_dict,
    create_key_to_list,
    merge_dicts,
)
from scubagoggles.custom_types import ApiReference
...
EVENTS = {
    ...
    "groups": ["CHANGE_APPLICATION_SETTING", "CREATE_APPLICATION_SETTING"],
    "meet": [
        "CHANGE_APPLICATION_SETTING",
        "CREATE_APPLICATION_SETTING",
        "DELETE_APPLICATION_SETTING",
...
class Provider:
        ...
        if self.domains is None:
            try:
                self.domains = (
                    ...
                    .domains()
                    .list(customer=self.customer_id)
                    .execute()["domains"]
                )
    ...
    def get_spf_records(self, domains: list) -> list:
        ...
        if n_low_confidence > 0:
            warnings.warn(
                f"get_spf_records: for {n_low_confidence} domain(s), \
    ...
    See ProviderSettingsExport.json under 'spf_records' for more details.",
                RuntimeWarning,
            )
        return results
    ...
    def get_dkim_records(self, domains: list) -> list:
        ...
        for domain in domains:
            ...
            if not result["HighConfidence"]:
                n_low_confidence += 1
            results.append(
                {"domain": domain, "rdata": result["Answers"], "log": log_entries}
            )

        if n_low_confidence > 0:
            warnings.warn(
                f"get_dkim_records: for {n_low_confidence} domain(s), \
    the traditional DNS queries returned an empty answer \
    section and the DoH queries failed. Will assume DKIM not configured, but \
    can't guarantee that failure isn't due to something like split horizon DNS. \
    See ProviderSettingsExport.json under 'dkim_records' for more details.",
                RuntimeWarning,
            )
        return results
    ...
    def get_dmarc_records(self, domains: list) -> list:
        ...
        if n_low_confidence > 0:
            warnings.warn(
                f"get_dmarc_records: for {n_low_confidence} domain(s), \
    the traditional DNS queries returned an empty answer \
    section and the DoH queries failed. Will assume DMARC not configured, but \
    can't guarantee that failure isn't due to something like split horizon DNS. \
    See ProviderSettingsExport.json under 'dmarc_records' for more details.",
                RuntimeWarning,
            )
        return results
    ...
    def get_super_admins(self) -> dict:
        ...
        try:
            response = (
                self.services["directory"]
                .users()
                .list(customer=self.customer_id, query="isAdmin=True")
                .execute()
            )
            admins = []
    ...
    def get_ous(self) -> dict:
        ...
        try:
            response = (
                self.services["directory"]
                .orgunits()
                .list(customerId=self.customer_id)
                .execute()
            )
            ...
            warnings.warn(
                f"Exception thrown while getting top level OU: {exc}", RuntimeWarning
            )
    ...
    def get_toplevel_ou(self) -> str:
        ...
        try:
            response = (
                self.services["directory"]
                .orgunits()
                .list(customerId=self.customer_id, orgUnitPath="/", type="children")
                .execute()
            )
            ...
            response = (
                self.services["directory"]
                .orgunits()
                .get(customerId=self.customer_id, orgUnitPath=parent_ou)
                .execute()
            )
    ...
    def get_tenant_info(self) -> dict:
        ...
        return {"domain": primary_domain, "topLevelOU": self.get_toplevel_ou()}
    ...
    def get_gws_logs(self, products: list, event: str) -> dict:
        ...
        response = (
            ...
            .activities()
            .list(userKey="all", applicationName="admin", eventName=event)
            .execute()
        ).get("items", [])
        ...
        subset_prod_to_app_name = {
            ...
            if prod in prod_to_app_name_values
        }
            ...
            for domain in domains:
                ...
                for group in response.get("groups"):
                    ...
                    group_settings.append(
                        group_service.groups().get(groupUniqueId=email).execute()
                    )
    ...
    def call_gws_providers(self, products: list, quiet) -> dict:
        ...
        try:
            ...
            for orgunit in product_to_items["organizational_units"][
                "organizationUnits"
            ]:
            ...
            warnings.warn(
                f"Exception thrown while getting tenant data: {exc}", RuntimeWarning
            )
        ...
        try:
            for event, product_list in events_to_products_bar:
                ...
                bar_descr = (
                    f"Running Provider: Exporting {event} events for {products}..."
                )
            ...
            warnings.warn(
                "Provider Exception thrown while getting the logs; "
                f"outputs will be incorrect: {exc}",
                RuntimeWarning,
            )
        ...
        try:
            ...
            if (
                "commoncontrols" in product_to_logs
