...
from scubagoggles.custom_types import API_LINKS

SCUBA_GITHUB_URL = "https://github.com/cisagov/scubagoggles"


def get_test_result(
    requirement_met: bool, criticality: str, no_such_events: bool
) -> str:
    """
...
def create_html_table(table_data: list) -> str:
    ...
    table_html = pd.DataFrame(table_data).to_html(
        border=0, index=False, escape=False, render_links=True
    )
...
def build_front_page_html(fragments: list, tenant_info: dict) -> str:
    ...
    with open(
        os.path.join(reporter_path, "./FrontPageReport/FrontPageReportTemplate.html"),
        mode="r",
        encoding="UTF-8",
    ) as file:
        html = file.read()
    ...
    with open(
        os.path.join(reporter_path, "./styles/main.css"), mode="r", encoding="UTF-8"
    ) as file:
        css = file.read()
...
def build_report_html(
    fragments: list, product: str, tenant_domain: str, main_report_name: str
) -> str:
    """
    ...
    reporter_path = str(rel_abs_path(__file__, "./"))
    with open(
        os.path.join(reporter_path, "./IndividualReport/IndividualReportTemplate.html"),
        mode="r",
        encoding="UTF-8",
    ) as file:
        html = file.read()
    ...
    with open(
        os.path.join(reporter_path, "scripts/main.js"), mode="r", encoding="UTF-8"
    ) as file:
    ...
    classroom_notification = (
        "<p>Note: Google Classroom is not available by default in GWS"
...
def get_failed_prereqs(
    test: dict, successful_calls: set, unsuccessful_calls: set
) -> set:
    """
    ...
    failed_prereqs = set().union(
        prereqs.difference(successful_calls), prereqs.intersection(unsuccessful_calls)
    )
...
def get_failed_details(failed_prereqs: set) -> str:
    ...
    if len(failed_apis) > 0:
        ...
        failed_details += (
            f"This test depends on the following API call(s) "
            f"which did not execute successfully: {links}. "
        )
    if len(failed_functions) > 0:
        failed_details += (
            f"This test depends on the following function(s) "
...
def rego_json_to_ind_reports(
    ...
    fragments = []
    json_data = []
    report_stats = {"Manual": 0, "Passes": 0, "Errors": 0, "Failures": 0, "Warnings": 0}
    ...
    for baseline_group in product_policies:
        ...
        for control in baseline_group["Controls"]:
            tests = [
                test for test in test_results_data if test["PolicyId"] == control["Id"]
            ]
            if len(tests) == 0:
                # Handle the case where Rego doesn't output anything for a given control
                report_stats["Errors"] += 1
                issues_link = (
                    f'<a href="{SCUBA_GITHUB_URL}/issues" target="_blank">GitHub</a>'
                )
                table_data.append(
                    {
                ...
                warnings.warn(
                    f"No test results found for Control Id {control['Id']}",
                    RuntimeWarning,
                )
            else:
                for test in tests:
                    failed_prereqs = get_failed_prereqs(
                        test, successful_calls, unsuccessful_calls
                    )
                    if len(failed_prereqs) > 0:
                        ...
                        if product_capitalized == "Rules":
                            ...
                            product_capitalized == "Commoncontrols"
        ...
        fragments.append(
            f"<h2>{product_upper}-{baseline_group['GroupNumber']} \
    ...
    html = build_report_html(
        fragments, prod_to_fullname[product], tenant_domain, main_report_name
    )
    with open(
